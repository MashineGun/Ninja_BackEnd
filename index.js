...
client.prototype.dataHandler = function dataHandler(device) {
	...
	return function(data) {
		...
		try {
			//self.log.info(self.getGuid(device) + ' ' + data);
			var request = require("request");
			request({
				uri: "http://localhost:8010/listen.php",
				method: "POST",
				form: {
					guid: self.getGuid(device),
					da: data,
					action: 'PUT',
			...
			//self.log.debug("Error sending data (%s) to custom back-end", self.getGuid(device));
		}
